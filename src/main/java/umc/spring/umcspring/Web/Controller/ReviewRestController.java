...
public class ReviewRestController {
    ...
    @Operation(summary = "특정 가게의 내가 작성한 리뷰 목록 API", description = "특정 가게의 내가 작성한 리뷰들의 목록을 조회하는 API이며, 페이징을 포함합니다. query String 으로 page 번호를 주세요")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "COMMON200", description = "OK, 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH003", description = "access 토큰을 주세요!", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH004", description = "access 토큰 만료", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
    ...
    @GetMapping("/store/{storeId}/members/{memberId}")
    public ApiResponse<ReviewResponseDTO.ReviewPreViewListDTO> getMyReviewList(
        ...
        return ApiResponse.onSuccess(
                ReviewConverter.reviewPreViewListDTO(
                        reviewQueryService.getMyReviewList(storeId, memberId, page)
                )
        );
    }
